...
async function hydrate(opts){opts=opts||{};var s=store();if(!s)return{ok:false,changed:false,pushed:0,pulled:0};var changed=false,pushed=0,pulled=0;applying=true;try{for(var i=0;i<KEYS.length;i++){var k=KEYS[i],remote=null;try{remote=await s.get(PREFIX+k,null,true)}catch(e){remote=null}var localRaw=raw(k);if(remote&&typeof remote==='object'&&Object.prototype.hasOwnProperty.call(remote,'removed')){if(remote.removed){if(localRaw!=null){origRemove.call(localStorage,k);changed=true;pulled++}}else{var rr=JSON.stringify(remote.value);if(localRaw!==rr){origSet.call(localStorage,k,rr);changed=true;pulled++}}}else if(localRaw!=null){try{await s.setNow(PREFIX+k,{value:parse(localRaw),removed:false,updatedAt:new Date().toISOString()});pushed++}catch(e){}}if(i%5===4)await wait(35)}}finally{applying=false}try{window.dispatchEvent(new CustomEvent('nursetrack:cloud-mirror-ready',{detail:{changed:changed,pushed:pushed,pulled:pulled}}))}catch(e){}if(changed&&opts.reload!==false)setTimeout(function(){location.reload()},80);return{ok:true,changed:changed,pushed:pushed,pulled:pulled}}
async function syncNow(){var s=store();if(!s)return{ok:false};status('Sincronizando…');if(queue.size)await wait(520);return hydrate({reload:true})}
async function init(){if(started)return;started=true;status('Verificando nube…');try{var r=await hydrate({reload:true});if(r.ok&&!r.changed)status('Nube ✓');else if(!r.ok)status('Guardado local')}catch(e){status('Guardado local')}}
