...
function notifyModules(){['nursetrack:templates-synced','nursetrack:social-templates-synced','nursetrack:addiction-coding-synced','nursetrack:membership-payment-synced','nursetrack:medication-interactions-synced'].forEach(function(n){try{window.dispatchEvent(new Event(n))}catch(e){}})}
async function push(showMessage){var c=client(),u=user();if(!c||!u||applying)return false;clearTimeout(pushTimer);pushTimer=null;var m=meta(),values=capture(),stamp=now();if(showMessage)status('Guardando…','busy');try{var r=await c.from(TABLE).upsert({user_id:u.id,app_data:{schema:6,modules:values,stamps:m},updated_at:stamp},{onConflict:'user_id'});if(r.error)throw r.error;remember(values);status('Nube sincronizada','ok');notifyModules();return true}catch(e){status('Sin conexión','error');return false}}
