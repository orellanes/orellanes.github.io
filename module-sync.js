...
var KEYS=[
  ...
  'nursetrack_v3_company_branding',
...
function status(text,kind){try{if(typeof window.nt28SetCloudStatus==='function')window.nt28SetCloudStatus(text,kind||'')}catch(e){}try{var p=document.getElementById('nt28CloudPanelStatus');if(p&&text==='Nube sincronizada')p.textContent='Datos, firmas, branding, usuarios, permisos, asignaciones, plantillas, medicamentos, membresía y pagos sincronizados.'}catch(e){}}
...
window.ntModuleSyncNow=syncNow;document.addEventListener('click',function(e){var t=e.target&&e.target.closest?e.target.closest('#nt28CloudSyncNow'):null;if(t)setTimeout(syncNow,50)},true);setInterval(detectLocalChanges,8000);setTimeout(detectLocalChanges,1000);
})();