...
function status(text,kind){try{if(typeof window.nt28SetCloudStatus==='function')window.nt28SetCloudStatus(text,kind||'')}catch(e){}try{var p=document.getElementById('nt28CloudPanelStatus');if(p&&text==='Nube sincronizada')p.textContent='Datos, plantillas de enfermería y Trabajo Social sincronizados.'}catch(e){}}
function remember(values){KEYS.forEach(function(k){lastLocal[k]=serial(values[k])})}
function notifyTemplates(){try{window.dispatchEvent(new Event('nursetrack:templates-synced'))}catch(e){}try{window.dispatchEvent(new Event('nursetrack:social-templates-synced'))}catch(e){}}
async function push(showMessage){var c=client(),u=user();if(!c||!u||applying)return false;clearTimeout(pushTimer);pushTimer=null;var m=meta(),values=capture(),stamp=now();if(showMessage)status('Guardando…','busy');try{var r=await c.from(TABLE).upsert({user_id:u.id,app_data:{schema:3,modules:values,stamps:m},updated_at:stamp},{onConflict:'user_id'});if(r.error)throw r.error;remember(values);status('Nube sincronizada','ok');notifyTemplates();return true}catch(e){status('Sin conexión','error');return false}}
function schedulePush(){clearTimeout(pushTimer);pushTimer=setTimeout(function(){push(false)},700)}
async function initialize(){var c=client(),u=user();if(!c||!u||initializing)return;if(activeUser===u.id)return;initializing=true;status('Sincronizando enfermería y Trabajo Social…','busy');try{var r=await c.from(TABLE).select('app_data,updated_at').eq('user_id',u.id).maybeSingle();if(r.error)throw r.error;var values=capture(),m=meta(),needPush=false,data=r.data&&r.data.app_data?r.data.app_data:null;if(data){var remote=data.modules||{},stamps=data.stamps||{},fallback=r.data.updated_at||'';applying=true;KEYS.forEach(function(k){var rv=Object.prototype.hasOwnProperty.call(remote,k)?remote[k]:undefined,rs=stamps[k]||fallback,ls=m[k]||'',lv=values[k];if(rv!==undefined&&rs&&(!ls||rs>ls)){writeValue(k,rv);m[k]=rs;values[k]=rv}else if(rv===undefined&&meaningful(lv)){if(!ls)m[k]=now();needPush=true}else if(rv!==undefined&&ls&&ls>=rs&&serial(lv)!==serial(rv))needPush=true});applying=false;saveMeta(m)}else{var first=now();KEYS.forEach(function(k){if(meaningful(values[k])&&!m[k])m[k]=first});saveMeta(m);needPush=true}activeUser=u.id;remember(capture());if(needPush)await push(false);else{status('Nube sincronizada','ok');notifyTemplates()}}catch(e){applying=false;status('Sin conexión','error')}initializing=false}
async function pull(){var c=client(),u=user();if(!c||!u||activeUser!==u.id||applying)return;try{var r=await c.from(TABLE).select('app_data,updated_at').eq('user_id',u.id).maybeSingle();if(r.error||!r.data||!r.data.app_data)return;var data=r.data.app_data,remote=data.modules||{},stamps=data.stamps||{},fallback=r.data.updated_at||'',m=meta(),changed=false;applying=true;KEYS.forEach(function(k){if(!Object.prototype.hasOwnProperty.call(remote,k))return;var rs=stamps[k]||fallback,ls=m[k]||'';if(rs&&(!ls||rs>ls)){if(writeValue(k,remote[k])){m[k]=rs;lastLocal[k]=serial(remote[k]);changed=true}}});applying=false;if(changed){saveMeta(m);try{if(typeof window.renderAll==='function')window.renderAll()}catch(e){}try{if(typeof window.ntBillingRender==='function')window.ntBillingRender()}catch(e){}try{window.dispatchEvent(new Event('nursetrack:medication-safety-updated'))}catch(e){}notifyTemplates();status('Nube sincronizada','ok')}}catch(e){applying=false}}
