...
function refreshControls(v){var map={apLogoText:'logoText',apLogoBg:'logoBg',apBrandText:'brandText',apSubtitleText:'subtitleText',apTopNavText:'topNavText',apTopBg:'topBg',apSideText:'sideText',apSideBg:'sideBg'};Object.keys(map).forEach(function(id){var a=document.getElementById(id),b=document.getElementById(id+'Hex');if(a)a.value=v[map[id]];if(b)b.value=v[map[id]]});var nums={apLogoSize:'logoSize',apBrandSize:'brandSize',apSubtitleSize:'subtitleSize',apTopNavSize:'topNavSize',apSideNavSize:'sideNavSize'};Object.keys(nums).forEach(function(id){var e=document.getElementById(id),o=document.getElementById(id+'Out');if(e)e.value=v[nums[id]];if(o)o.textContent=v[nums[id]]+' px'})}
...
function setPreset(x,label){var next=Object.assign({},read(),x);write(next);refreshControls(next);document.getElementById('apMsg').textContent='✓ '+label+' aplicado sin salir de NurseTrack.'}
document.getElementById('apWhite').onclick=function(){setPreset({logoText:'#ffffff',brandText:'#ffffff',subtitleText:'#e6f4ff',topNavText:'#ffffff',sideText:'#ffffff',topBg:'#073b78',sideBg:'#073b78'},'Blanco alto contraste')};
document.getElementById('apBlue').onclick=function(){setPreset({logoText:'#ffffff',logoBg:'#1d6fd6',brandText:'#ffffff',subtitleText:'#dfeeff',topNavText:'#ffffff',sideText:'#ffffff',topBg:'#083f86',sideBg:'#083f86'},'Azul clásico')};
document.getElementById('apTeal').onclick=function(){setPreset({logoText:'#ffffff',logoBg:'#267f86',brandText:'#ffffff',subtitleText:'#def4f4',topNavText:'#ffffff',sideText:'#ffffff',topBg:'#155c68',sideBg:'#155c68'},'Verde azulado')};
document.getElementById('apReset').onclick=function(){localStorage.removeItem(KEY);apply(DEF);refreshControls(DEF);document.getElementById('apMsg').textContent='✓ Apariencia restaurada sin salir de NurseTrack.'};apply(v)}
