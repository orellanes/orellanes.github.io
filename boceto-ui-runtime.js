...
function brand(){const a=appearance();qa('.brand h1').forEach(h=>h.textContent=a.brandLabel||'NurseTrack');qa('.brand small').forEach(s=>s.textContent=('subtitleLabel' in a)?a.subtitleLabel:'Pacientes. Cuidado. Resultados.');qa('.logo').forEach(l=>l.textContent=a.logoLabel||'NT');if(window.NT_APPEARANCE_SETTINGS&&window.NT_APPEARANCE_SETTINGS.apply)try{window.NT_APPEARANCE_SETTINGS.apply(a)}catch(e){}}
...
function install(){addStyle();brand();nav();flow();document.documentElement.dataset.ntBocetoUi='3'}
function boot(){install();const mo=new MutationObserver(()=>{clearTimeout(window.__ntBocetoTimer);window.__ntBocetoTimer=setTimeout(install,100)});mo.observe(document.body,{childList:true,subtree:true});window.addEventListener('nursetrack:appearance-updated',()=>setTimeout(install,0))}
