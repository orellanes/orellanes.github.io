...
function msg(text){const area=document.getElementById('patientActionArea');if(area)area.innerHTML='<div class="notice">'+String(text||'')+'</div>'}
...
async function openNursing(){
  ...
  try{
    ...
    if(!window.NT_V4_NURSING_FAST||typeof window.NT_V4_NURSING_FAST.open!=='function')throw new Error('La plantilla de Enfermería no terminó de iniciar');
    window.NT_V4_NURSING_FAST.open();
  }catch(e){
...
async function openSocial(){
  ...
  }finally{openingSocial=false}
}

document.addEventListener('click',function(e){
  const nursing=e.target.closest?.('.patient-action[data-action="nursing"],#newVisitBtn');
  if(nursing){e.preventDefault();e.stopImmediatePropagation();openNursing();return}
  ...
  if(mod){
    ...
    if(name==='Enfermería'||name==='Trabajo Social'){
      ...
      if(!patientPanel||patientPanel.classList.contains('hidden')){
        ...
        const st=document.getElementById('searchStatus');if(st){st.textContent='Busca y abre primero el expediente del paciente.';st.className='status'}
        return;
      }
