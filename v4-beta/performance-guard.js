...
async function load(force){
  ...
  loading=(async()=>{
    ...
    guard=normalize(r.data?.settings?.performance_guard);
    ready=true;
    updateCards();
    return guard;
...
function requireFeature(feature,label){
  ...
  toast((label||FEATURES[feature]?.label||'Función de alto consumo')+' está restringida por Rendimiento protegido.');
  return false;
}
async function save(next){
  const c=await context();
  if(!c.superAdmin)throw new Error('Solo el Súper Administrador puede cambiar este control.');
  const normalized=normalize(next);
  const r=await sb.rpc('nursetrack_set_performance_guard',{p_company_id:c.company,p_guard:normalized});
  if(r.error)throw r.error;
  guard=normalize(r.data||normalized);ready=true;updateCards();return guard;
}
function style(){
  if($('ntV4PerfStyle'))return;
  const s=document.createElement('style');s.id='ntV4PerfStyle';s.textContent=`
  .ntv4-perf-card{position:relative;border:1px solid #a7ced0!important;background:linear-gradient(180deg,#f8fdfd,#edf8f8)!important}
  .ntv4-perf-card .ntv4-perf-state{display:inline-block;margin-top:7px;padding:4px 8px;border-radius:999px;font-size:11px;font-weight:900;background:#dff4e8;color:#216b48}
  .ntv4-perf-card[data-open="1"] .ntv4-perf-state{background:#fff0d7;color:#8a5700}
  .ntv4-perf-modal{position:fixed;inset:0;z-index:2147483646;background:rgba(15,37,42,.42);display:grid;place-items:center;padding:18px}
  .ntv4-perf-box{width:min(720px,100%);max-height:88vh;overflow:auto;background:#fff;border-radius:20px;padding:20px;box-shadow:0 24px 70px rgba(0,0,0,.28);color:#17343c}
  .ntv4-perf-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.ntv4-perf-head h3{margin:0 0 5px}.ntv4-perf-close{border:0;background:#edf5f6;border-radius:10px;padding:8px 11px;font-weight:900}
  .ntv4-perf-banner{margin:14px 0;padding:12px 14px;border-radius:12px;background:#edf8f8;border:1px solid #cce5e5}.ntv4-perf-list{display:grid;gap:9px}
  .ntv4-perf-row{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;border:1px solid #d8e6e8;border-radius:12px;padding:12px}.ntv4-perf-row small{display:block;color:#6f858b;margin-top:3px}.ntv4-perf-toggle{min-width:108px;border:0;border-radius:10px;padding:9px 11px;font-weight:900;background:#edf5f6;color:#31545b}.ntv4-perf-toggle.on{background:#fff0d7;color:#8a5700}.ntv4-perf-master{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}.ntv4-perf-note{font-size:13px;color:#6f858b}
  @media(max-width:560px){.ntv4-perf-row{grid-template-columns:1fr}.ntv4-perf-toggle{width:100%}}
